...
# JLECmd Pipeline Runner for dfir-agentic
# Generates a CSV from Windows Jump Lists (.automaticDestinations-ms / .customDestinations-ms)
...
OUT_CSV_DIR="${PROJECT_ROOT}/outputs/csv/jlecmd/${RUN_ID}"
OUT_JSON_DIR="${PROJECT_ROOT}/outputs/jsonl/jlecmd/${RUN_ID}"

mkdir -p "${OUT_CSV_DIR}" "${OUT_JSON_DIR}"

# Find JLECmd executable
JLECMD_BIN=$(which jlecmd || which JLECmd || which JLECmd.exe || echo "JLECmd")
...
echo "INFO: Generating JLECmd CSV"
echo "  RUN_ID:   ${RUN_ID}"
...
# 2. Run JLECmd
# Note: Zimmerman tools output files formatted as: <timestamp>_JLECmd_Output.csv
set +e
"${JLECMD_BIN}" "${TARGET_ARG}" "${TARGET_PATH}" --csv "${OUT_CSV_DIR}" -q >"${STDOUT_LOG}" 2>"${STDERR_LOG}"
RC=$?
set -e

if [[ $RC -ne 0 ]]; then
  echo "FAIL: JLECmd failed (rc=${RC})" >&2
  echo "  stderr: ${STDERR_LOG}" >&2
  exit 2
fi

# Find the generated CSV (Zimmerman tools prepend datetimes)
GENERATED_CSV=$(find "${OUT_CSV_DIR}" -name "*JLECmd_Output.csv" | head -n 1)

# Depending on exact outputs or older versions the name format might vary
if [[ -z "${GENERATED_CSV}" ]]; then
  GENERATED_CSV=$(find "${OUT_CSV_DIR}" -name "*JLECmd*.csv" | head -n 1)
fi
...
ADAPTER_CONTRACT="${PROJECT_ROOT}/adapters/jlecmd_csv_to_findings_contract.py"
...
manifest = {
  ...
  "pipeline": {"name": "jlecmd", "version": "0.1.0"},
  "status": "ok",
  "inputs": {
  ...
  "latest": {
    "outputs/jsonl/jlecmd/LATEST": True,
  }
}
print(json.dumps(manifest, indent=2))
PY

LATEST_FILE="${PROJECT_ROOT}/outputs/jsonl/jlecmd/LATEST"
mkdir -p "${PROJECT_ROOT}/outputs/jsonl/jlecmd"

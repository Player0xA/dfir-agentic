...
def main() -> int:
    ...
    out_case_summary = intake_dir / "case_summary.md"
    out_case_summary.write_text("\n".join(md_lines), encoding="utf-8")
    print(f"OK: wrote {out_case_summary}")

    # Register in manifest
    case_manifest_path = intake_dir / "case_manifest.json"
    case_manifest = {
        "intake_id": intake_id,
        "artifacts": {
            "findings_json": {"path": str(out_path)},
            "summary_md": {"path": str(out_case_summary)}
        }
    }
    write_json(case_manifest_path, case_manifest)
    print(f"OK: wrote {case_manifest_path}")

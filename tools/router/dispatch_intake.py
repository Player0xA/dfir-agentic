...
def main() -> int:
    ...
    ap.add_argument("--selected-tools", type=str, help="Comma-separated list of tools to run (enforces exact tool selection)")
    args = ap.parse_args()
    ...
    elif raw_data.get("selected_tools"):
        selected_tools = raw_data["selected_tools"]
    ...
    for step in steps_to_run:
        ...
        # ENFORCE TOOL SELECTION: Skip if not selected
        if not should_run_tool(pipeline_id):
            step_history.append(f"{pipeline_id}: skipped (not selected)")
            print(f"\n>> Skipping playbook step: {pipeline_id} (not in selected tools)")
            continue
        ...
        if pipeline_id == "plaso_evtx":
            ...
            cmd = [str(script), plaso_run_id, utc_now_z(), evidence_path, str(case_dir), intake_id]
        else:
            cmd = [str(script), evidence_path, intake_id]

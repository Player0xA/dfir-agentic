...
def main() -> int:
    ...
    ap.add_argument("--action", required=True, choices=["dispatch_pipeline", "read_path", "run_enrichment"])
    ap.add_argument("--pipeline-id", default=None)
    ap.add_argument("--enrichment-id", default=None)
    ...
    if args.action == "run_enrichment":
        if not args.enrichment_id:
            return deny("run_enrichment requires --enrichment-id", 2)
        allowed = set(caps.get("can_run_enrichments", []))
        if args.enrichment_id not in allowed:
            return deny(f"{args.agent_id} cannot run enrichment '{args.enrichment_id}'")
        print("ALLOW")
        return 0
